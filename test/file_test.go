...
func TestGenerateChunkFile(t *testing.T) {
	...
	// 只读方式打开文件
	myFile, err := os.OpenFile("test.png", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
	// 存放每一次的分片数据
...
func TestMergeChunkFile(t *testing.T) {
	myFile, err := os.OpenFile("test1.png", os.O_CREATE|os.O_WRONLY|os.O_APPEND, os.ModePerm)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCheck(t *testing.T) {
	// 获取第一个文件的信息
	f1, err := os.OpenFile("test.png", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
	...
	// 获取第二个文件的信息
	f2, err := os.OpenFile("test1.png", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
