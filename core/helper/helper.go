...
// GenerateToken 调用库的NewWithClaims(加密方式,载荷).SignedString(签名) 生成token
func GenerateToken(id int, identity, name string, second int) (string, error) {
	...
	tokenString, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString([]byte(define.JwtKey))
...
func MailSendCode(mail, code string) error {
	e := email.NewEmail()
	e.From = "Get <xiaodao_yin@163.com>"
	e.To = []string{mail}
	e.Subject = "验证码发送测试"
	e.HTML = []byte("你的验证码为：<h1>" + code + "</h1>")
	err := e.SendWithTLS("smtp.163.com:465", smtp.PlainAuth("", "xiaodao_yin@163.com", define.MailPassword, "smtp.163.com"),
...
func CosUpload(r *http.Request) (string, error) {
	...
	key := "go-cloud-disk/" + UUID() + path.Ext(fileHeader.Filename)
